...
//   const getMyProfile=  useSelector(state=>state.getMyProfile)
  const [getMyProfile, setgetMyProfile] = useState(null);
  ...
  const anotherAsyncFunc= async()=>{
    ...
    const response = await fetch(
      ...
      const funct = async () => {
        ...
        var objectURL = URL.createObjectURL(myBlob);
        setgetProfilePic(objectURL);
      
        }else{
          setgetProfilePic("https://paragon.learnfacts.xyz/default-profile-picture.png");
